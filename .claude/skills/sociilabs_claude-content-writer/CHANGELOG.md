...
**Documentation regenerated in Simplified Technical English**
- Rewrote `README.md` and all nine files in `docs/` (four setup guides, the headless/MCP guide, the feature comparison, and three manual-test checklists) in ASD-STE100 STE style: active voice, one idea per sentence, plain words, no semicolons, no em dashes.
- Every regenerated file passes the STE prose gate (`node scripts/ste-lint.js --gate=blog <file>`): zero banned words, zero marketing adjectives, zero em dashes.
- Updated all download links to the v2.4.1 release and version references to v2.4.x.
- Documented the STE gate in each guide: the tiers, the hard-zero rule, the per-platform behavior (linter on Claude Code, the MCP server, and the Claude AI skill; manual self-lint on ChatGPT and Gemini), and the new `ste_gate` and `ste_per100w` state fields.
- Added a "Headless / MCP setup" link and an STE gate row to the README.
...
**ASD-STE100 Simplified Technical English (STE) writing law**
- New `references/ste-writing-rules.md` defines how every sentence is formed for all content types: active voice, one idea per sentence, plain words (use not utilize, help not facilitate), a verb not a nominalization, no phrasal verbs, no semicolons, no contractions.
- STE governs the Execute phase and is enforced at the Verify phase.

**STE compliance linter and mandatory verify gate**
- New dependency-free Node linter `scripts/ste-lint.js`, ported from the reference Python linter (same MARKETING / BANNED / PHRASAL word lists, regexes, and scoring). Adds a `--gate=<type>` tier resolver, `--self-test`, and module exports.
- `/writer:verify` runs the linter as a mandatory, blocking gate. Content that fails does not ship.
- Tiered thresholds preserve brand voice: strict (SEO metadata, operational email — zero violations), prose (blog, web, sales, case studies, newsletters — <= 3.0 violations per 100 words), social (LinkedIn, Twitter/X, Facebook, Instagram — <= 4.0 per 100 words). Marketing adjectives and banned words fail on every tier.
- `/writer:ship` refuses to ship unless the STE gate passed.
- New `npm run lint:ste` script.

### Changed

- Added `ste_gate` and `ste_per100w` fields to the project-state schema (`skills/state-schema.md`). `verify-writer-commands.js` now validates 30 required keys and self-tests the STE linter as part of `npm run verify`, which gates every release.
- Embedded the STE law and gate across all adapters: Claude Code, Claude AI skill (linter bundled in the skill's `scripts/`), ChatGPT and Gemini (manual self-lint; Gemini merges the law into `anti-ai-checklist.md` to stay within the 10-source limit), and the MCP server (returns an `ste_gate` error on failure).
- `dist/` build artifacts are now gitignored.
...
- Create unlimited writer profiles for different brands, clients, or voices
- Per-project profile assignment (each Claude Project can use different profile)
- Profile management commands: create, list, switch, view, edit, delete
- Profile storage in Claude memory (Claude AI) or files (Claude Code)

**Enhanced Platform Adapters**
- Updated ChatGPT Custom GPT instructions with multi-profile support
- Updated Gemini Gem instructions with multi-profile support  
- Added natural language triggers for all platforms
- Auto-advance support: "next", "continue", "proceed" to move between phases

**Ship Script**
- Automated release process: `npm run ship [patch|minor|major]`
- Builds distribution zips for all platforms
- Updates version in package.json and CHANGELOG
- Creates git tags and optionally publishes to npm

### Changed

**Claude AI Skill**
- Switched from file-first to memory-first storage model
- Added artifact output for all content
- Added `/writer:profile-list` command
- Added `/writer:profile-use [name]` command for switching profiles
- Condensed description to meet 1024 character limit

**Documentation**
- Complete rewrite of main README with download links
- Added platform comparison table
- Added multi-profile documentation across all setup guides
- Updated manual test checklists with multi-profile tests
...
**Skill files rewritten with production-grade step-by-step instructions**

All command skill files were expanded from minimal stubs into precise, multi-step guides:

- `writer:execute` — Rewrote from 10 lines to 100+ lines. Now covers section-by-section writing process, anti-AI discipline applied during writing (not just after), CTA shortcode insertion, and platform-specific formatting defaults
- `writer:ship` — Expanded from 20 lines to 180 lines. Adds numbered output directory structure (`001-slug.md`), YAML frontmatter template, and platform-specific publishing notes section
- `writer:profile-create` — Rewrote from form-style checklist to conversational interview flow. Adds URL scanning analysis step, 10-topic questionnaire structure, and file output paths (`PROFILE.md`, `PRODUCTS.md`, `CTAS.md`, `CASE-STUDIES.md`)
- `writer:discuss` — Rewrote for conversational approach. Adds audience awareness stages (unaware / problem-aware / solution-aware / product-aware), framework selection logic with rationale, one-paragraph brief format instead of a data table
- `writer:plan`, `writer:verify`, `writer:help`, `writer:next`, `writer:update`, `writer:status`, `writer:profile-edit`, `writer:profile-view`, `writer:profile-delete` — All updated with more precise instructions

**Reference files significantly expanded**

- `references/anti-ai-checklist.md` — Grew from 150 lines to 660 lines. Added scientific background on perplexity and burstiness as detection signals, era-specific AI vocabulary lists (GPT-4 era / GPT-4o era / 2025–2026 models), 25+ named pattern categories with before/after examples, and three-pass audit process
- `references/content-frameworks.md` — Grew from 450 lines to 950 lines. Added framework selection decision guide, new frameworks (PASTOR, SCQA, StoryBrand, Thought Companionship), audience awareness stage mapping, when-not-to-use guidance for each framework
- All platform convention files updated: `email-content-conventions.md`, `web-content-conventions.md`, `sales-content-conventions.md`, `twitter-conventions.md`, `facebook-conventions.md`, `instagram-conventions.md`, `seo-meta-conventions.md`, `profile-management.md`, `research-workflow.md`

**SKILL.md trimmed and refocused**

- Stripped from 1,200+ lines to ~230 lines
- Removed verbose auto-update check flow (handled by `writer:update` skill)
- Added reference file routing table: maps content type to the correct reference file to load
- Improved skill trigger description to catch casual content requests ("write me a LinkedIn post about X")

**README rewritten for developers**

- Removed marketing copy, testimonials, and verbose section descriptions
- Replaced with concise: what it does, who it's for, install, quick start, 5-phase table, and dependency table
- Tone shift from promotional to technical/functional

### Removed

- Stale planning artifacts: `.planning/IMPLEMENTATION-SUMMARY.md`, `.planning/MILESTONE-V3.md`, `.planning/PROGRESS-V2.md`
- `README.md.backup`
