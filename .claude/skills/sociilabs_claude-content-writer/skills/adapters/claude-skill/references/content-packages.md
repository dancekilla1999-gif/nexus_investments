...
When producing a content package (blog + LinkedIn + newsletter), follow these format-specific conventions. All defaults can be overridden by the user's preferences stored in the writer profile files (`content-writer-output/profile/PROFILE.md`, `PRODUCTS.md`, `CTAS.md`, `CASE-STUDIES.md`).
